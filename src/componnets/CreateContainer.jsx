...
const CreateContainer = () => {
    ...
    const uploadImg = (e) => {
        ...
        uploadTask.on('state_changet', (snapshot) => {
            ...
            getDownloadURL(uploadTask.snapshot.ref).then(downloadURL => {
                ...
                setIsLoading(false)
                setFields(true)
                setMsg('Image uploaded succssfuly')
    ...
    const deleteImage = () => {
        ...
        deleteObject(deleteRef).then(() => {
            ...
            setIsLoading(false)
            setFields(true)
            setMsg('Image deleted succssfuly')
            setAlertStatus('success')
            setTimeout(() => {
                setFields(false)
            },7000)
    ...
    const saveDetails = () => {
        ...
        try {
            ...
            setFields(true)
            setMsg("Reguired fields can't be empty")
            setAlertStatus('danger')
            setTimeout(()=> {
                setFields(false)
                setIsLoading(false)
                ...
                setIsLoading(false)
                setFields(true)
                setMsg('Data uploaded succssfuly')
                setAlertStatus('success')
                clearData()
                setTimeout(() => {
                    setFields(false)
        ...
        <div className='w-full min-h-screen flex items-center justify-center'>
            <div className='w-[90%] md:w-[75%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4'>
                ...
                <div className="w-full">
                    ...
                    >
                        <options value='other' className='bg-white'>Select Category</options>
                ...
                <div className="group flex justify-center items-center flex-col border-2 border-dotted border-gray-300 w-full h-225 md:h-420 cursor-pointer rounded-lg">
                    {isLoading ? <Loader /> :
                        <>
                            {!imageAsset ?(
                                <>
                                <label className="w-full h-full flex flex-1 items-center justify-center cursor-pointer">
                                    <div className="w-full h-full flex flex-1 items-center justify-center gap-2">
                                        <MdCloudUpload className="text-gray-500 text-3xl hover:text-gray-700" />
                                        <p className='text-gray-500 hover:text-gray-700' >Clich here to upload</p>
                                    </div>
